      ...
      <View style={this.props.containerStyle}>
        <View style={{ flexDirection: this.props.flexDirection }}>
          {this.state.radioButtons.map(data => (
            <RadioButton key={data.label} data={data} onPress={this.onPress} labelStyle={this.props.labelStyle} marginLeft={this.props.marginLeft} />
          ))}
        </View>
      </View>
    )
...
class RadioButton extends Component {
  render () {
    ...
    let margin = { marginLeft: this.props.marginLeft }
    ...
    return (
      ...
      >
        ...
        <Text style={[{ alignSelf: 'center' }, margin, this.props.labelStyle]}>{data.label}</Text>
      </TouchableOpacity>
    )
  }
}
